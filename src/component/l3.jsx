...
function ParticleSystem() {
  ...
  useFrame((state) => {
    ...
    if (isLoading) {
      ...
      expansionProgress.current = Math.min(expansionProgress.current + 0.005, 1)
      const currentScale = THREE.MathUtils.lerp(INITIAL_SCALE, 1, expansionProgress.current)

      visibleParticleCount = Math.floor(THREE.MathUtils.lerp(INITIAL_PARTICLE_COUNT, FULL_PARTICLE_COUNT, Math.pow(expansionProgress.current, 2)));
...
export default function LoadingAudioReactiveParticles() {
  ...
  return (
      <Canvas camera={{ position: [0, 0, 2] }}>
        ...
        <EffectComposer>
          <Bloom luminanceThreshold={0.3} luminanceSmoothing={0.7} height={200} />
        </EffectComposer>
        <ParticleSystem />
      </Canvas>
  )
